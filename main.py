...
if __name__ == "__main__":
    ...
    net.load_state_dict(torch.load("model4"))

    while True:
        gameStr = sys.stdin.readline()
        if not gameStr:
            break
        ...
        with torch.no_grad():
            outputs = net(torch.unsqueeze(torch.from_numpy(board).float().to(device), 0))
            print(outputs.shape)

            while True:
                _, netTurn = torch.max(outputs, 1)
                if curBoard[netTurn // 15, netTurn % 15] != 0:
                    outputs[:, netTurn] = 0
                    continue
                else:
                    break

        a, b = check(curBoard, 1)
        if a != -1 and b != -1:
            sys.stdout.write(to_word((a, b)) + '\n')
            sys.stdout.flush()
            break

        sys.stdout.write(to_word((netTurn // 15, netTurn % 15)) + '\n')
        sys.stdout.flush()
